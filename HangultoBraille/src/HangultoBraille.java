...
public class HangultoBraille {
    ...
    private static ArrayList<Object> letter(String HangulLetter) {
        ...
        if (HangulLetter.equals("것")) {
            result.add(HangulLetter);
            result.add(Arrays.deepToString(MATCH_H2B_GRAMMAR.get('것')));
            return result;
        }
        ...
        if (hangul_decomposed.length() >= 3 && MATCH_H2B_GRAMMAR2.containsKey(hangul_decomposed.substring(1, 3))) {
            if (MATCH_H2B_CHO.containsKey(hangul_decomposed.charAt(0))) {
                result.add(hangul_decomposed.charAt(0));
                result.add(Arrays.deepToString(MATCH_H2B_CHO.get(hangul_decomposed.charAt(0))));
            }
            result.add(hangul_decomposed.substring(1, 3));
            result.add(Arrays.deepToString(MATCH_H2B_GRAMMAR2.get(hangul_decomposed.substring(1, 3))));
            return result;
        }
    ...
    // '안녕'에 대해 '안'과'녕' 하나하나씩만 letter함수 인자로 보냄
    public static ArrayList<ArrayList<Object>> text(String HangulSentence) {
        ...
        for (char HangulLetter : HangulSentence.toCharArray()) {
            ...
            ResultList.add(letter(HangulLetterStr));
        } //한글sentence를 한글자씩 쪼개서 letter 함수에 인자로 보냄 ex) 안녕 = 안,녕

        return ResultList;
    }


    public static void PrintBraille(String Hangul) {
        ArrayList<ArrayList<Object>> Result = text(Hangul);  //긴 한글(문장,단어)이 들어올 경우
